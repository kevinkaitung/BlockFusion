# Note: model created at 6/28,
# to test VAE with some non-upsampling/downsampling layers and no KL-reg
...
encoder_dims =         [128, 512, 512,  1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024,  1024,  vae_config["z_shape"][0]]
feature_size_encoder = [128,  64,  32,   16,   16,    8,    8,    4,    8,    8,   16,   16,   vae_config["z_shape"][1]]

encoders_down_end_idx = 7
encoders_up_end_idx = 12

# decoder_in_channels = 128
decoder_dims =         [512, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 512, 512, vae_config["plane_shape"][1]]
feature_size_decoder = [ 32,  16,   16,    8,    8,    4,    8,    8,   16,    16,  32,  64, vae_config["plane_shape"][2]]

decoders_down_end_idx = 5
decoders_up_end_idx = 12

# these indices index for encoder_dims/decoder_dims
fpn_encoders_layer_dim_idx = [2, 4, 6]
fpn_decoders_layer_dim_idx = [0, 2, 4, 5]

# these indices index for the group of blocks (i.e., encoders_down, ...) in block_config
fpn_encoders_down_idx = [1, 3, 5]
fpn_encoders_up_idx = [1, 3]
fpn_decoders_down_idx = [-1, 1, 3, 4]
fpn_decoders_up_idx = [1, 3, 5]
