# Note: model created at 11/29,
# model_j is used to compare to model_g with extra upsampling blocks (back to original resolution) in encoder
...
encoder_dims =         [128, 256, 512, 1024, 1024, 1024, 1024, 1024, 512, 256,  2 * vae_config["z_shape"][0]]
feature_size_encoder = [128,  64,  32,   16,    8,    4,    8,   16,  32,  64,  vae_config["z_shape"][1]]
# Note: None is just the placeholder to keep index align with other arrays
encoder_use_transformers = [None, False, True, True, True, True, True,  True,  True, True, False]
encoder_use_resblocks = [None, True, False, False, False, False, False, False, False, False, True]
encoder_num_resblocks = 1

encoders_down_end_idx = 5
encoders_up_end_idx = 10

# decoder_in_channels = 128
decoder_dims =         [128, 256, 512,  1024, 1024, 1024, 1024, 1024, 512, 256, vae_config["plane_shape"][1]]
feature_size_decoder = [128,  64,  32,    16,    8,    4,    8,   16,  32,  64, vae_config["plane_shape"][2]]
decoder_use_transformers = [None, True, True, True, True, True, True, True, True, True, False]
decoder_use_resblocks = [None, False, False, False, False, False, False, False, False, False, True]
decoder_num_resblocks = 1

decoders_down_end_idx = 5
decoders_up_end_idx = 10

# these indices index for encoder_dims/decoder_dims
fpn_encoders_layer_dim_idx = [1, 2, 3, 4]
fpn_decoders_layer_dim_idx = [1, 2, 3, 4]
