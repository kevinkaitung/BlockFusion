# Note: model created at 7/1,
# for new VAE architecture 
# (the arrangement of down/upsampling layers, resblock, and transformer block more aligns with that in original LDM repo)
...
encoder_dims =         [128, 256, 256,  512,  512, 1024,  512,  512,  2 * vae_config["z_shape"][0]]
...
encoder_use_transformers = [None, False, False, False, False, False, True, True, False]
encoder_use_resblocks = [None, True, True, True, True, True, True, True, True]
...
decoder_dims =         [256,  512,  512, 1024,  512,  512, 256, 256, vae_config["plane_shape"][1]]
feature_size_decoder = [ 32,   16,    8,    4,    8,   16,  32,  64, vae_config["plane_shape"][2]]
decoder_use_transformers = [None, False, False, False, False, False, True, True, False]
decoder_use_resblocks = [None, True, True, True, True, True, True, True, True]
...
for i in range(encoders_down_end_idx):
    block_config["encoders_down"].append({
        ...
        "is_batchnorm": True if i == (encoders_down_end_idx - 1) else False
    })
...
for j, i in enumerate(range(encoders_down_end_idx, encoders_up_end_idx)):
    block_config["encoders_up"].append({
        ...
        "is_decoder_output": False,
        "num_res_blocks": encoder_num_resblocks,
        "is_batchnorm": True if i == (encoders_up_end_idx - 1) else False
    })
...
for i in range(decoders_down_end_idx):
    block_config["decoders_down"].append({
        ...
        "is_batchnorm": True if i == (decoders_down_end_idx - 1) else False
    })
...
for j, i in enumerate(range(decoders_down_end_idx, decoders_up_end_idx)):
    block_config["decoders_up"].append({
        ...
        "num_res_blocks": decoder_num_resblocks,
        "is_batchnorm": True if i == (decoders_up_end_idx - 1) else False
    })
