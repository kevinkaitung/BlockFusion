...
def inference(dataset, data_res, chunk_size, value_range, net, instances_to_store, filename_prefix, sampled_batch_size=10000000):
    psnr_list = []
    psnr_fast_list = []
    ssim_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_instances', type=int, default=150, help="Number of light dirs to use for evaluation")
    parser.add_argument('--selected_light_dirs_file_path', type=str, default="selected_light_dirs.json")
    ...
    if args.selected_light_dirs_file_path:
        ...
        # all_lighting_dirs_cartesian = np.array(selected_light_dirs["light_dir_cartesian"])
        all_lighting_dirs_cartesian = selected_light_dirs["light_dir_cartesian"][:n_instances]
    ...
    for config_file_path, triplane_file_path, recon_type in zip(args.configs, args.triplane_file_paths, args.triplane_recon_types):
        ...
        dataset = RandomlyGenerateLightDir(
            ...
            light_dir_spherical=cartesian_to_spherical_coords(np.array(all_lighting_dirs_cartesian)).tolist()
        ...
        net = nn.ModuleList([NeurCompNet(n_input_dims=3, n_output_dims=config.n_labels, bias=False, 
                                n_hidden_layers=config.n_layers, n_neurons=config.n_hid, 
                                is_residual=True).cuda()])
        ...
        psnr_list = inference(dataset, data_res, chunk_size, value_range, net, args.instances_to_store, recon_type)
        psnr_lists.append(psnr_list)
        # light_dir_lists.append(loaded_model['light_dir_cartesian'])
        # the light dirs to compare are this one, not the one from the loaded model
        light_dir_lists.append(all_lighting_dirs_cartesian)
