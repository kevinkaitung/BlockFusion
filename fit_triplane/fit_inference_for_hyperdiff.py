# this script is used to plot the recon quality across all instances and find the one with the highest quality
# recognize it as the one hyperdiffusion tried to generate (because currently don't incorporate conditioning in hyperdiffusion)
...
def inference(dataset, data_res, chunk_size, value_range, triplane, net, instances_to_store, filename_prefix, sampled_batch_size=10000000):
    ...
    with torch.no_grad():
        ...
        ### section end
        this_batch_triplane = triplane[0].cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--light_dir_path', type=str)
    ...
    for config_file_path, triplane_file_path, recon_type in zip(args.configs, args.triplane_file_paths, args.triplane_recon_types):
        ...
        with open(f"{args.light_dir_path}", "r") as f:
            loaded_lights = json.load(f)
        
        n_instances = len(loaded_model['light_dir_cartesian'])
        n_lights = len(loaded_lights['light_dir_cartesian'])
        ...
        dataset = RandomlyGenerateLightDir(
            sampler=sampler,
            n_instances=n_lights,
            tfn=args.tfn_file_path,
            sample_batch_size=config.sample_batch_size,
            light_dir_spherical=cartesian_to_spherical_coords(np.array(loaded_lights['light_dir_cartesian'])).tolist()
        ...
        light_dir_lists.append(loaded_lights['light_dir_cartesian'])
