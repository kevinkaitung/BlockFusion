...
if __name__ == "__main__":
    ...
    parser.add_argument('--offset', type=int, default=150, help="Previous number of shadow volumes")
    ...
    # return as np array
    all_lighting_dirs_cartesian = fibonacci_sphere(args.n_instances, True)
    ...
    for idx in range(args.n_instances // config.batch_size):
        ...
        torch.save({
            ...
            'offset': start_idx + args.offset,
            'end_idx': end_idx + args.offset,
            # 'lr_net': final_lr_net,
            # 'lr_tri': final_lr_tri,
            'triplane_resolution': current_tri_reso,
            'epoch': epoch,
        }, os.path.join(run_dir, f"triplane_offset_{start_idx + args.offset}.ckpt"))
    ...
    num_epoch_to_reload_new_subset = config.max_iters
    ...
    for run in range(num_run):
        ...
        for offset_idx, offset in enumerate(permuted):
            # need to shift by the previous #instances
            ...
            # if run == num_run - 1:
            # if offset_idx > 0:
            # TODO: already assume only training each subset in one run (instead of they taking turns)
            # so, just create optimizer from scratch 
            # and later need to clean up old code
            ...
            for epoch in tqdm(range(start_epoch + 1, start_epoch + num_epoch_to_reload_new_subset + 1)):
                ...
                if epoch in config.c2f_scale:
                    ...
                    # don't create new optimizer for net here
                    optimizer = create_optimizer(None, triplane, config, args.optimizer_type)
            ...
            if offset_idx == 0:
                ...
                torch.save({
                                'net_state_dict': loaded_pretrained_mlp['net_state_dict'],
                                ...
                                # 'optimizer_state_dict': optimizer.state_dict(),
                                # concat with old lists of light dirs
                                'light_dir_spherical': loaded_pretrained_mlp['light_dir_spherical'] + all_lighting_dirs_spherical,
                                'light_dir_cartesian': loaded_pretrained_mlp['light_dir_cartesian'] + all_lighting_dirs_cartesian,
                                'permuted_indices': loaded_pretrained_mlp['permuted_indices'] + (permuted_indices + args.offset).tolist(),
                                # 'lr_net': final_lr_net,
                                'epoch': epoch,
