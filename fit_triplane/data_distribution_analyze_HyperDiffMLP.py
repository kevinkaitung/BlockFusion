...
from fit_shadow_subset_training_HyperDiffMLP import HyperDiffusionMLP
...
if __name__ == "__main__":
    ...
    for config_file_path, SIREN_file_path, recon_type, output_activation in zip(config_files, SIREN_file_paths, SIREN_recon_types, output_activations):
        ...
        nets = [HyperDiffusionMLP(out_size=config.n_labels, hidden_neurons=[config.n_hid] * config.n_layers, 
                                                  use_leaky_relu=True, use_bias=False, multires=10, output_type=None, move=False).cuda() for _ in range(n_instances)]
