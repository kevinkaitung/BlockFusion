...
from networks import HyperDiffusionMLP
...
if __name__ == "__main__":
    ...
    sample_net = nn.ModuleList([HyperDiffusionMLP(out_size=config.n_labels, hidden_neurons=[config.n_hid] * config.n_layers, 
                                                  use_leaky_relu=True, use_bias=False, multires=10, output_type=None, move=False).cuda()])
    ...
    for idx in range(args.n_instances // config.batch_size):
        ...
        nets = [
            ...
            HyperDiffusionMLP(out_size=config.n_labels, hidden_neurons=[config.n_hid] * config.n_layers, use_leaky_relu=True, use_bias=False, multires=10, output_type=None, move=False).cuda()
